...
public class CommandFramework implements CommandExecutor {
	...
	private games.bevs.library.modules.playerdata.PlayerDataHandler<?> playerDataHandler;

	/**
	 * Initializes the command framework and sets up the command maps
	 *
	 * Note: playerDataHandler maybe null, if need be
	 */
	public CommandFramework(Plugin plugin, PlayerDataHandler<?> playerDataHandler) {
		this.plugin = plugin;
	...
	public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (command.consoleOnly() && (sender instanceof Player)) {
					sender.sendMessage(CC.red + "This command is only performable by the console");
					return true;
				}
				try {
					method.invoke(methodObject, new CommandArgs(sender, cmd, label, args,
							cmdLabel.split("\\.").length - 1,  this.playerDataHandler));
				} catch (IllegalArgumentException e) {
					e.printStackTrace();
				} catch (IllegalAccessException e) {
					e.printStackTrace();
		...
		defaultCommand(new CommandArgs(sender, cmd, label, args, 0,  this.playerDataHandler));
		return true;
	}
	...
	public void registerCompleter(String label, Method m, Object obj) {
		...
		if (map.getCommand(cmdLabel) instanceof BukkitCommand) {
			...
			if (command.completer == null) {
				command.completer = new BukkitCompleter(this.playerDataHandler);
			}
			...
			try {
				...
				if (field.get(command) == null) {
					BukkitCompleter completer = new BukkitCompleter(this.playerDataHandler);
